  ...
  return (
    <Link href={`/product/${product.id}`} className="block h-full">
      <motion.article
        ...
        whileHover={{ y: -6 }}
        className="group flex h-full flex-col overflow-hidden rounded-[28px] border border-white/[0.08] bg-white/[0.035] p-3 backdrop-blur-xl transition-all duration-300 hover:border-[#1D976C]/40 hover:bg-white/[0.055] hover:shadow-[0_20px_60px_rgba(29,151,108,0.12)]"
      >
        <div className="relative h-[330px] overflow-hidden rounded-[22px] bg-[#0A110E]">
          <div className="absolute inset-0 z-0 bg-[radial-gradient(circle_at_center,rgba(147,249,185,0.10),transparent_68%)]" />

          {isEmoji ? (
            <div className="flex h-full w-full items-center justify-center text-[100px]">
              {product.image}
            </div>
          ) : (
            <img
          ...
          {product.isOrganic && (
            <span className="absolute left-3 top-3 z-20 inline-flex items-center gap-1 rounded-full border border-[#93F9B9]/20 bg-black/40 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-[#93F9B9] backdrop-blur-md">
        ...
        <div className="flex flex-1 flex-col px-3 pb-3 pt-5">
          <div className="mb-2 flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-[#7D8D86]">{product.unit}</span>
          </div>
          ...
          <div className="mt-auto flex items-end justify-between gap-4">
            <div>
              ...
              <span className="mt-0.5 block text-xs text-[#65736D]">per {product.unit}</span>
            </div>
